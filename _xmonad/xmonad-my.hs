...
myWorkspaces  = ["1:main","2:chat","3:web","4:emacs","5:media","6", "7", "8"]

	 --onWorkspace "4:Pidgin" pidginLayout $ onWorkspaces ["2:chat", "7:media"] nobordersLayout $ tiled1 ||| Mirror tiled1 ||| nobordersLayout
myLayout = onWorkspace "5:media" nobordersLayout $ tiled1 ||| Mirror tiled1 ||| nobordersLayout
  where
...
main = xmonad defaultConfig
     -- xmproc <- spawnPipe "/usr/bin/xmobar /home/morazow/.xmobarrc" --spawn "xcompmgr -Cf"
     --xmonad $ azertyConfig
     	    { manageHook = manageDocks <+> myManageHook <+> manageHook defaultConfig -- make sure to include myManageHook definition from above
